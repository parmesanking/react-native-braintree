...
class BrainTreeDropIn: NSObject, PKPaymentAuthorizationViewControllerDelegate {
    ...
    func startPayPalCheckout(_ clientToken: String,
        ...
        if let agreementDesc = agreementDescription {
            request.billingAgreementDescription = agreementDesc // Displayed in customer's PayPal account
        }
        payPalDriver.tokenizePayPalAccount(with: request) { (tokenizedPayPalAccount, error) -> Void in
            DispatchQueue.main.async {
                if let tokenizedPayPalAccount = tokenizedPayPalAccount {
                    ...
                    if let adr = tokenizedPayPalAccount.billingAddress {
                        billingAddress = [
                            "recipientName": adr.recipientName,
                            "streetAddress": adr.streetAddress,
                            "extendedAddress": adr.extendedAddress,
                            "locality": adr.locality,
                            "countryCodeAlpha2": adr.countryCodeAlpha2,
                    ...
                    if let shipAdr = tokenizedPayPalAccount.shippingAddress {
                        shippingAddress = [
                            "recipientName": shipAdr.recipientName,
                            "streetAddress": shipAdr.streetAddress,
                            "extendedAddress": shipAdr.extendedAddress,
                            "locality": shipAdr.locality,
                            "countryCodeAlpha2": shipAdr.countryCodeAlpha2,
                            "postalCode": shipAdr.postalCode,
                            "region": shipAdr.region,
                        ]
                    }
                    
                    let data: [String: Any] = [
                        "nonce": tokenizedPayPalAccount.nonce,
                        "type": tokenizedPayPalAccount.type,
                        "isDefault": tokenizedPayPalAccount.isDefault,
                        "email": tokenizedPayPalAccount.email,
                        "firstName": tokenizedPayPalAccount.firstName,
                        "lastName": tokenizedPayPalAccount.lastName,
                        "phone": tokenizedPayPalAccount.phone,
                        "clientMetadataID": tokenizedPayPalAccount.clientMetadataID,
